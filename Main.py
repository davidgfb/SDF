        ...
        self.quad, self.program = quad_fs(),\
            ...
            float map(in vec3 p) { //sdf quad               
                ...
                return sqrt((sign(dot(cross(v21, nor), p1)) + 
                       ...
                       dot(nor, p1) * dot(nor, p1) / dot2(nor)) - 0.5; //0.01 truco    
            }

            float map1(vec3 p, vec3 or = vec3(0),\
            ...
            float get_Impacto(float dist, vec3 rd, vec3 ro) {
                vec3 impacto = get_P(dist, rd, ro);
                
                //return opUnion(map(impacto), map1(impacto));
                return opSubtraction(map(impacto), map1(impacto));
            }

            void main() { //color basado en la posicion del pixel en la pantalla 
                vec3 col = vec3(0), ro = vec3(0, 0, -1),
                     rd = normalize(vec3((2 * gl_FragCoord.xy - u_resolution.xy) /
                         u_resolution.y, 1)); //inicia negro, centra el origen de la pantalla, origen_Rayo, dir_Rayo

                float dist = 0;

                for (int i = 0; i < 256; i++) {
                    float hit = get_Impacto(dist, rd, ro); //vector p, distancia mas corta al obj
                    dist += hit;

                    if (dist > 100 || abs(hit) < 1e-4) i = 256; //rompe bucle cuando esta lo suf cerca del obj o cuando el rayo escapa de la escena
                }

                if (dist < 100) col += (get_Impacto(dist, rd, ro) + 1) / 2; //suma la distancia al color resultante
